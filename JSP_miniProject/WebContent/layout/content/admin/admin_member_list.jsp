...
$(document).ready(function(){
	$("#viewwrite").change(function(){
		...
		location.href = "adminfrom.team3?pagenum=1&viewwrite="+value;
	});
});
</script>
<%
...
<table class="boardList" style="width:100%;">
	...
	<tr>
		...
		%>
			<tr class="writeList">
				<td><a href="memmodify.team3?memnum=<%= list.get(i).getMemNum() %>"><%= list.get(i).getRowNum() %></a></td>
				<% if(list.get(i).getaLive() == 0) {%>
				<td><input type="button" value="승인" onclick="location.href='changealive.team3?number=<%= list.get(i).getMemNum() %>&gopage=adminfrom'"></td>
				<% }else{ %>
				<td>완료</td>
				<% } %>
...
<%
	for(int i = 1; i <= total_page ; i ++){
		if(i == total_page){
%>
			<span><a href="adminfrom.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a></span>
<%
		}else{
%>
			<span><a href="adminfrom.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a> | </span>
