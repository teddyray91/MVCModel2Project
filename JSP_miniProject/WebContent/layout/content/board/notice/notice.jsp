...
$(document).ready(function(){
	$("#viewwrite").change(function(){
		...
		location.href = "noticeboard.team3?pagenum=1&viewwrite="+value;
	});
});
</script>
<%
	...
	String admin= "";
	if(session.getAttribute("admin") == null) admin = "0";
	else admin = session.getAttribute("admin").toString();
...
<div class="boardFunc">
	<button onclick="loginOk('<%= loginCheck %>', 'notice', '<%= admin %>')">글쓰기</button>
</div>
<div align="center">
	<p class="page_num">
<%
	for(int i = 1; i <= total_page ; i ++){
		if(i == total_page){
%>
			<span><a href="noticeboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a></span>
<%
		}else{
%>
			<span><a href="noticeboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a> | </span>
