...
$(document).ready(function(){
	$("#viewwrite").change(function(){
		...
		location.href = "freeboard.team3?pagenum=1&viewwrite="+value;
	});
});
</script>
<%
...
<div class="boardFunc">
	<button onclick="loginOk('<%= loginCheck %>', 'freeboard')">글쓰기</button>
</div>
...
<%
	for(int i = 1; i <= total_page ; i ++){
		if(i == total_page){
%>
			<span><a href="freeboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a></span>
<%
		}else{
%>
			<span><a href="freeboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a> | </span>
