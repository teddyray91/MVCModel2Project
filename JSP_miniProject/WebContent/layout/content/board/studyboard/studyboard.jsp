...
$(document).ready(function(){
	$("#viewwrite").change(function(){
		...
		location.href = "studyboard.team3?pagenum=1&viewwrite="+value;
	});
});
</script>
<%
...
<div class="boardFunc">
	<button onclick="loginOk('<%= loginCheck %>', 'studyboard')">글쓰기</button>
</div>
<div align="center">
	<p class="page_num">
<%
	for(int i = 1; i <= total_page ; i ++){
		if(i == total_page){
%>
			<span><a href="studyboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a></span>
<%
		}else{
%>
			<span><a href="studyboard.team3?totalpage=<%= total_page %>&pagenum=<%= i %>&viewwrite=<%= viewwrite %>"><%= i %></a> | </span>
